...
describe("fetchCurrentTemperature", () => {
  ...
  it("correctly rejects with the right string when latitude is invalid", () => {
    const promise = fetchCurrentTemperature({lat: -181, lon: 0});
    return promise.catch(result => {
      assert(result === "Received status code 400: Bad Request");
    });
  });

  it("correctly rejects with the right string when longitude is invalid", () => {
    const promise = fetchCurrentTemperature({lat: 0, lon: -181});
    return promise.catch(result => {
      assert(result === "Received status code 400: Bad Request");
    });
  });

  it("correctly rejects with the right string when both coords are invalid", () => {
    const promise = fetchCurrentTemperature({lat: -91, lon: -181});
    return promise.catch(result => {
      assert(result === "Received status code 400: Bad Request");
    });
  });
